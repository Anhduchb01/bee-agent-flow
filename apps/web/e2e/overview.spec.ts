...
test("các khối chính, đúng thứ tự đã duyệt", async ({ page }) => {
  ...
  for (const name of ["Claude", "Active sessions", "Last seven days"]) {
...
test("hạn mức hiện thanh phần trăm và đồng hồ tới cửa sổ mới", async ({ page }) => {
  const five = page.getByRole("progressbar", { name: "5-hour limit" });
  await expect(five).toBeVisible();
  await expect(five).toHaveAttribute("aria-valuenow", "38");
