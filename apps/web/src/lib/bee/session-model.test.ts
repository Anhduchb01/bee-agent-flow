...
import { changeSessionModel } from "./session-ctl";
import { readSessionIn } from "./sessions-fs";
...
describe("changeSessionModel — V2.7 model switch", () => {
  beforeEach(async () => {
  ...
  it("writes the alias into session.json (unit not running → applies next start)", async () => {
    await writeSession();
    expect(await changeSessionModel(ID, "opus[1m]")).toEqual({ ok: true });
  ...
  it("chat sessions may switch model too — a model is not a tool", async () => {
    ...
    expect(await changeSessionModel(ID, "haiku")).toEqual({ ok: true });
  ...
  it("refuses an id or alias outside the allowlist — nothing is written", async () => {
    ...
    expect((await changeSessionModel(ID, "gpt-4" as never)).ok).toBe(false);
    expect((await changeSessionModel(ID, "claude-fable-5" as never)).ok).toBe(false);
    expect((await changeSessionModel("not-a-uuid", "opus")).ok).toBe(false);
  ...
  it("a session that never chose one reads back as default", async () => {
    await writeSession();
    expect((await readSessionIn(dir, ID))?.model).toBe("default");
    await fs.writeFile(
