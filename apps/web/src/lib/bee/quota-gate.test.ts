...
describe("checkQuota — the brake (FR-3.3)", () => {
  it("well under the threshold: opens, and says nothing alarming", () => {
    const k = checkQuota(usage(), { nguong: 85, luc: LUC });
  ...
  it("over the threshold on the 5h window: refuses, and names the window + reset time", () => {
    ...
    expect(k.reason).toMatch(/16:20|1h ?20|80 phút/);
  });
  ...
  it("exactly at the threshold still opens — the rule is 'over', not 'at'", () => {
    expect(checkQuota(usage({ five_hour: { percent: 85, resets_at: null } }), { nguong: 85, luc: LUC }).moDuoc).toBe(true);
    expect(checkQuota(usage({ five_hour: { percent: 86, resets_at: null } }), { nguong: 85, luc: LUC }).moDuoc).toBe(false);
  });

  it("never measured → opens, and SAYS it is flying blind", () => {
    const k = checkQuota(null, { nguong: 85, luc: LUC });
    expect(k.moDuoc).toBe(true);
  ...
  it("stale numbers → still opens, but the reason admits the brake is untrustworthy", () => {
    ...
    const k = checkQuota(usage({ fetched_at: "2026-08-24T04:00:00Z" }), { nguong: 85, luc: LUC });
    expect(k.moDuoc).toBe(true);
  ...
  it("a threshold of 0 disables the brake — an escape hatch that is explicit", () => {
    const k = checkQuota(usage({ five_hour: { percent: 99, resets_at: null } }), { nguong: 0, luc: LUC });
    expect(k.moDuoc).toBe(true);
  });
});
