...
describe("openSession — the brake sits at the door every new session goes through", () => {
  ...
  it("over the threshold: refuses BEFORE writing anything to disk", async () => {
    await setQuota(93);
    const outcome = await openSession({ slug: "myapp", num: 1, repo: "you/myapp", title: null, worktree: true });

    expect(outcome.ok).toBe(false);
    if (outcome.ok) return;
    expect(outcome.message).toMatch(/93%/);
  ...
  it("under the threshold: the brake does not get in the way", async () => {
    ...
    // lib/bee/ctl.ts.
    if (!outcome.ok) expect(outcome.message).not.toMatch(/hạn mức/);
  ...
  it("QUOTA_BRAKE_PCT tunes the line", async () => {
    ...
    const outcome = await openSession({ slug: "myapp", num: 1, repo: "you/myapp", title: null, worktree: true });
    expect(outcome.ok).toBe(false);
    if (!outcome.ok) expect(outcome.message).toMatch(/50%/);
  });
  ...
  it("Continue on an existing session is NOT braked — it resumes, it does not open", async () => {
    ...
    expect(outcome.ok).toBe(false);
    if (outcome.ok) return;
    expect(outcome.message).toMatch(/BEE_CTL=none/);
    expect(outcome.message).not.toMatch(/hạn mức/);
  });
