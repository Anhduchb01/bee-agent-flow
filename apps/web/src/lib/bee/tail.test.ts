...
describe("docTiep", () => {
  it("dòng JSON bị cắt đôi giữa hai lần đọc không bao giờ bị phát nửa dòng", async () => {
    ...
    const lan1 = await docTiep(f, 0, "");
    expect(lan1.dong).toEqual(['{"type":"a"}']);
    expect(lan1.rest).toBe('{"type":"b","x":');

    appendFileSync(f, '1}\n');
    const lan2 = await docTiep(f, lan1.offset, lan1.rest);
    expect(lan2.dong).toEqual(['{"type":"b","x":1}']);
  ...
  it("không có gì mới thì trả offset và rest nguyên vẹn", async () => {
    ...
    const lan1 = await docTiep(f, 0, "");
    const lan2 = await docTiep(f, lan1.offset, lan1.rest);
    expect(lan2).toEqual({ dong: [], offset: lan1.offset, rest: "" });
  });

  it("file không tồn tại là 'chưa có gì mới', không phải lỗi", async () => {
    const ket = await docTiep("/khong/ton/tai/run.jsonl", 5, "dở");
    expect(ket).toEqual({ dong: [], offset: 5, rest: "dở" });
  });
});
