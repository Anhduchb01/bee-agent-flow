...
export function sevenDaySummary(days: RunDay[]): string | null {
  const todayRow = days.at(-1);
  const prev = days.slice(0, -1);
  const prevTotal = prev.reduce((n, d) => n + d.total, 0);
  const prevError = prev.reduce((n, d) => n + d.err, 0);

  if (!todayRow || (todayRow.total === 0 && prevTotal === 0)) return null;
  if (todayRow.total === 0) return "No runs today yet.";

  const rateToday = todayRow.err / todayRow.total;
  const ratePrev = prevTotal === 0 ? 0 : prevError / prevTotal;

  if (todayRow.err > 0 && rateToday > ratePrev * 2) {
    return `${todayRow.err} of ${todayRow.total} runs failed today — sharply higher than the six days before.`;
  }
  return `${todayRow.finished} of ${todayRow.total} runs finished today.`;
}
