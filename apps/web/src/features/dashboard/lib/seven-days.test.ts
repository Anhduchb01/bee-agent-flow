...
describe("lastSevenDays", () => {
  ...
  it("đếm đúng xong và lỗi theo ngày", () => {
    const days = lastSevenDays([...chay(0, "ok", 7), ...chay(0, "fail", 6), ...chay(2, "ok", 3)], NOW);
  ...
  it("mọi result khác 'ok' đều tính là lỗi", () => {
    const days = lastSevenDays([...chay(0, "gave-up"), ...chay(0, "timeout"), ...chay(0, "ok")], NOW);

    expect(days.at(-1)).toMatchObject({ xong: 1, loi: 2 });
  });

  it("bỏ qua lần chạy ngoài cửa sổ bảy ngày", () => {
    const days = lastSevenDays(chay(30, "ok", 5), NOW);
  ...
  it("nhãn thứ đúng theo lịch", () => {
    ...
    expect(lastSevenDays([], NOW).map((d) => d.nhan)).toEqual([
      "Fri",
      "Sat",
      "Sun",
      "Mon",
...
describe("tomTatBayNgay", () => {
  it("gọi tên khi hôm nay tệ hơn hẳn", () => {
    const days = lastSevenDays([...chay(0, "ok", 7), ...chay(0, "fail", 6), ...chay(3, "ok", 12)], NOW);
  ...
  it("nói rõ khi hôm nay chưa chạy gì", () => {
    expect(tomTatBayNgay(lastSevenDays(chay(2, "ok", 4), NOW))).toBe("No runs today yet.");
  });
