...
export function BoardTable({ muc }: { muc: MucBang[] }) {
  return (
    <div className="overflow-hidden rounded-card border border-border bg-card">
      <div className="hidden border-b border-border px-4 py-2 text-xs text-muted-foreground sm:grid sm:grid-cols-[minmax(0,1fr)_180px_200px_110px] sm:gap-4">
        <span>Issue</span>
        <span>Session</span>
      ...
      <ul>
        {muc.map((m) => (
          <li
            ...
            className="flex flex-col gap-2 border-b border-border px-4 py-3 last:border-b-0 sm:grid sm:grid-cols-[minmax(0,1fr)_180px_200px_110px] sm:items-center sm:gap-4"
          >
            ...
            <span className="text-xs text-body">{NHAN_LANE[m.lane]}</span>
          </li>
        ))}
      </ul>
    </div>
