...
Thiết kế chi tiết ở **[reconciler.md](reconciler.md)**.
...
Cái giá: **thêm ~1,5–2 ngày** để tự dựng những thứ Actions cho không (log, secret, timeout, chống trùng), và ba chế độ hỏng riêng — nguy hiểm nhất là *reconciler chết im lặng*. Đối chiếu đầy đủ ở [reconciler.md §9](reconciler.md#9-đối-chiếu-mất-gì-được-gì) và §13.
...
| Điều phối | **Polling reconciler** ([chi tiết](reconciler.md)) | Hàng đợi ưu tiên, `--resume`, không mất việc, không khoá GitHub | Actions runner (ít công hơn nhưng không có 4 điểm trên) · Webhook (mất event + cần inbound) |
...
Danh sách đầy đủ cho phương án C ở [reconciler.md §11](reconciler.md#11-danh-sách-file). Tóm tắt:
